...
| Migration journal present | Desktop startup pauses an interrupted, unactivated conversion and opens the original database; an activated descriptor opens its committed generation |
...
1. A logical read acquires a bounded slot from its database manager and pins the immutable files before beginning a SQLite read transaction. Candidate selection, counts, staged bytes and payload locators share that transaction. Nested reads reuse its connection and file lease.
2. Projected Parquet decoding uses a bounded cache and reads only the selected records. File identity, checksum and selected projection identify cache entries. Independent readers retain separate SQLite transactions while the existing writer continues recording and publishing files.
3. The catalog revision identifies cached query results. Ordinary capture and metadata writes advance it; readers already holding a snapshot retain the earlier values. A separate persisted revocation revision advances on logical deletion, privacy replacement and policy changes.
4. A completed read releases its transaction and checks revocation under the generation gate before response admission. The manager reserves read-pool capacity for this check. Cancellation interrupts the SQLite worker and completes rollback before returning its connection and slot.
5. Admission transfers a prepared body to the response transport and releases the gate. File pins remain alive until decoding and response preparation finish. Upload/export adapters retain their existing revision tokens through serialization and admission.

Frame projections and per-frame element records share the bounded immutable cache in every hybrid build. Cache locks cover lookup and publication; checksums, decoding and allocation cleanup run outside them. Readers of the same file and projection share one in-flight result, while cache hits proceed independently of occupied decoder slots. Owner shutdown cancels workers, closes admission, drains leases and transports, and releases the database-manager lease before another generation opens.
...
FTS element search selects IDs, frame references, order, kind and visibility through a compact resident SQLite lookup. Frame browsing uses the existing grouped counts and Parquet metadata projection. Normal startup builds this lookup once for older hybrid generations in resumable bounded batches; new migrations and element writes maintain it as records arrive. A materialized page of selected IDs then retrieves the complete returned records. Range scans seek through the indexed file catalog and retrieve one file reference at a time. Metadata and complete-record projections share one cache bounded by the configured decoded-byte budget; checksum and schema checks apply to each newly decoded file.
...
Migration is an explicitly invoked, resumable conversion with a single durable activation point. Recording and background mutation pause for the conversion. An app-wide modal owns interaction while the native operation runs, through restart and verification. The modal follows only in-memory activity, so reopening the app restores ordinary access. The migration journal defines these phases:
...
| `paused` | Desktop restarted before activation; original storage accepts recording and queries; resume builds a fresh candidate from current history |
| `active` | Active descriptor durably selects the candidate |
| `complete` | Verified candidate selected; original database retained with its identity and completion receipt |

1. **Prepare and freeze.** Resolve the exact source root, verify its health/capabilities, acquire lifecycle ownership after recorder shutdown, and record the journal. The coordinated source owner freezes committed SQLite/WAL history for direct reads. The original remains intact until its separate deletion action.
2. **Build.** Create an empty index with the source schema and migration provenance, then initialize its hybrid catalog and indexes. Stream source rows in stable-key order into byte-bounded batches, preserving IDs, rowids, nulls, values, relationships, vectors, mutable state, source bindings/checkpoints, and privacy stamps. Seal each batch through the normal verified Parquet publication protocol before importing the next. The new index enables incremental vacuum before schema creation, reclaims released batch pages in place, and checkpoints construction WAL between batches. The migration needs space for the resulting generation plus bounded batch/WAL work and the configured disk reserve; admission checks available space against the reserve plus two file-byte budgets before each batch. Interrupted unpublished attempts rebuild from the frozen source; desktop resume includes history recorded since restart.
3. **Index.** Hybrid insert paths construct search postings, payload locators, and relationship/count summaries from each bounded batch. Retained search indexes preserve their indexed records. Existing application triggers are installed for subsequent writes after historical import. Source migration records, original sequences, and archive-writer versions travel with the constructed generation.
4. **Verify during conversion.** Compare each inserted batch with its source while the rows are still in the construction transaction, including row IDs, SQLite value types, nulls, text/JSON, vectors, mutable state, and media paths. Accumulate table receipts during this pass. Each Parquet publication already decodes its new files and checks complete value parity before sealing. These checks establish the migration receipt as records move into their final storage. The source file identity protects interrupted attempts; full database audits remain available through the explicit `verify` command.
5. **Prepare activation.** Synchronize candidate files/catalog and directories, close candidate handles, and reopen through the normal hybrid opener to validate schema identity, catalog completeness, and file presence. Retrieve the first and last frame payloads and compare up to two sampled search pages with the source. Persist `ready` with the verification receipts while source recording remains paused.
...
7. **Complete and retain.** After activation and successful reopening, persist the verification receipt and original-file identity, mark `complete`, and retire the journal. The source database remains on disk. The app restarts against the selected generation and confirms it can serve history.
8. **Delete original, when requested.** A separate destructive action requires explicit confirmation after successful migration and app switchover. The running manager must match the active descriptor and completion receipt; an altered original, SQLite WAL, or rollback journal blocks deletion. An orphaned shared-memory index contains no database records. Remove only the retained original database and synchronize its directory. Shared media remains referenced at its existing location.

Startup resolves the descriptor before opening a writer. Desktop startup acquires lifecycle ownership to mark an interrupted, unactivated journal `paused`, then opens the original database for ordinary use. Resuming replaces that journal's unpublished candidate and takes a fresh snapshot, including history recorded since restart. Offline CLI resumes retain their frozen-source checks. If activation committed before the journal advanced, descriptor identity establishes `active`. After activation, recovery uses the candidate and finishes its completion receipt; new writes always belong to that generation. The original remains available until the user explicitly deletes it; a failed deletion keeps the active database usable.
