...
async fn schema_staging_and_reclamation_interruptions_retain_completed_files() {
    for (point, hit) in [
        ("migration_schema_step",2), ("migration_schema_step",8),
        ("migration_schema_step",16), ("migration_schema_step",32),
        ("migration_batch_staged",2), ("seal_committed",2),
        ("bulk_committed",2), ("migration_blocks_reclaimed",2),
    ] {
        let root = tempfile::tempdir().unwrap();
        fixture(root.path()).await;
        let crashed = std::process::Command::new(env!("CARGO_BIN_EXE_screenpipe-storage"))
            .arg("migrate").arg(root.path()).env("SCREENPIPE_STORAGE_CRASH_AT",point)
            .env("SCREENPIPE_STORAGE_CRASH_HIT",hit.to_string()).output().unwrap();
        assert_eq!(crashed.status.code(),Some(86),"{point}/{hit}: {}",String::from_utf8_lossy(&crashed.stderr));
        let before: Vec<_> = screenpipe_db::storage::inventory(root.path()).unwrap().into_iter()
            .filter(|p| p.extension().is_some_and(|s| s=="parquet"))
            .map(|p| { let bytes=std::fs::read(&p).unwrap(); (p,bytes) }).collect();
        assert!(screenpipe_db::storage::migration_requires_resume(root.path()).unwrap());
        let report = migrate(root.path(),Default::default(),Default::default()).await.unwrap();
        assert_eq!(report.frames,48);
        for (path,bytes) in before { assert_eq!(std::fs::read(path).unwrap(),bytes,"completed Parquet must not be rebuilt at {point}/{hit}"); }
        let db = DatabaseManager::new(root.path().join("db.sqlite").to_str().unwrap(),Default::default()).await.unwrap();
        db.verify_storage().await.unwrap();
        assert_eq!(sqlx::query_scalar::<_,i64>("SELECT count(*) FROM elements").fetch_one(&db.pool).await.unwrap(),48);
        db.close().await;
    }
}
...
async fn privacy_pending_payloads_remain_resident_and_searchable() {
    let root = tempfile::tempdir().unwrap();
    fixture(root.path()).await;
    let source = DatabaseManager::new(root.path().join("db.sqlite").to_str().unwrap(),Default::default()).await.unwrap();
    ...
    source.close().await;
    let options = MigrationOptions { privacy: screenpipe_db::storage::PrivacyPolicy { identity:"required-redaction".into(),required_surfaces:15 },..Default::default() };
    migrate(root.path(),Default::default(),options).await.unwrap();
    let db = DatabaseManager::new(root.path().join("db.sqlite").to_str().unwrap(),Default::default()).await.unwrap();
    assert_eq!(sqlx::query_scalar::<_,i64>("SELECT count(*) FROM frame_payloads WHERE state='staged'").fetch_one(&db.pool).await.unwrap(),48);
    assert_eq!(sqlx::query_scalar::<_,i64>("SELECT count(*) FROM _bulk_element_rows").fetch_one(&db.pool).await.unwrap(),48);
    assert_eq!(sqlx::query_scalar::<_,i64>("SELECT count(*) FROM elements_fts WHERE elements_fts MATCH 'searchable'").fetch_one(&db.pool).await.unwrap(),48);
    assert_eq!(db.seal_frame_payloads().await.unwrap(),0);
    for table in ["audio_transcriptions", "ui_events"] {
        assert_eq!(sqlx::query_scalar::<_,i64>(sqlx::AssertSqlSafe(format!("SELECT count(*) FROM {table}_fts WHERE {table}_fts MATCH 'private'"))).fetch_one(&db.pool).await.unwrap(),1);
    }
    db.execute_raw_sql_write("UPDATE pipe_executions SET stdout='still running' WHERE id=1").await.unwrap();
    assert_eq!(sqlx::query_scalar::<_,String>("SELECT stdout FROM pipe_executions WHERE id=1").fetch_one(&db.pool).await.unwrap(),"still running");
    db.verify_storage().await.unwrap();
    db.close().await;
}
...
async fn unsupported_volume_fails_before_conversion() {
    ...
    options.budget.record_bytes=1024*1024;
    let error=migrate(root.path(),Default::default(),options).await.unwrap_err();
    assert!(error.to_string().contains("filesystem reclamation unavailable"),"{error}");
    assert!(root.path().join("db.sqlite").is_file());
    assert!(!root.path().join("storage-migration.json").exists());
    let db = DatabaseManager::new(root.path().join("db.sqlite").to_str().unwrap(),Default::default()).await.unwrap();
    assert_eq!(sqlx::query_scalar::<_,i64>("SELECT count(*) FROM frames").fetch_one(&db.pool).await.unwrap(),48);
    db.close().await;
}
...
async fn failed_payload_write_resumes_without_discarding_published_files() {
    ...
    let result = screenpipe_db::storage::migrate_with_progress(root.path(),Default::default(),Default::default(),|p| {
        let mut changed=changed.lock().unwrap();
        if p.completed_records.unwrap_or(0)==0 || !changed.is_empty() { return; }
        let journal:serde_json::Value=serde_json::from_slice(&std::fs::read(root.path().join("storage-migration.json")).unwrap()).unwrap();
        let directory=root.path().join(journal["descriptor"]["payloads"].as_str().unwrap());
        let permissions=std::fs::metadata(&directory).unwrap().permissions();
        std::fs::set_permissions(&directory,std::fs::Permissions::from_mode(0o500)).unwrap();
        changed.push((directory,permissions));
    }).await;
    for (path,permissions) in changed.into_inner().unwrap() {std::fs::set_permissions(path,permissions).unwrap();}
    assert!(result.is_err());
    assert!(screenpipe_db::storage::migration_requires_resume(root.path()).unwrap());
    let files:Vec<_>=screenpipe_db::storage::inventory(root.path()).unwrap().into_iter().filter(|p|p.extension().is_some_and(|e|e=="parquet")).map(|p|{let data=std::fs::read(&p).unwrap();(p,data)}).collect();
    assert!(!files.is_empty());
    migrate(root.path(),Default::default(),Default::default()).await.unwrap();
    for (p,data) in files {assert_eq!(std::fs::read(p).unwrap(),data);}
}
...
async fn exhausted_volume_resumes_after_space_is_restored() {
    ...
    options.budget.record_bytes=1024*1024;
    options.budget.disk_reserve_bytes=0;
    let result=screenpipe_db::storage::migrate_with_progress(root.path(),Default::default(),options,|p| {
        if p.completed_records.unwrap_or(0)==0 || filled.swap(true,std::sync::atomic::Ordering::SeqCst) {return;}
        ...
        while fs2::available_space(&volume).unwrap()>2*1024*1024 {
            if file.write_all(&block).and_then(|_|file.sync_all()).is_err() {break;}
        }
    }).await;
    assert!(result.is_err());
    assert!(screenpipe_db::storage::migration_requires_resume(root.path()).unwrap());
    assert!(DatabaseManager::new(root.path().join("db.sqlite").to_str().unwrap(),Default::default()).await.is_err());
    let files:Vec<_>=screenpipe_db::storage::inventory(root.path()).unwrap().into_iter().filter(|p|p.extension().is_some_and(|e|e=="parquet")).map(|p|{let data=std::fs::read(&p).unwrap();(p,data)}).collect();
    assert!(!files.is_empty());
